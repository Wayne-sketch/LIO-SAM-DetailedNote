 ...
 *        传入一帧原始特征点云 放入哈希表+八叉树的数据结构中
 * 
 * @param feat_map 平面特征或线特征哈希表 键是体素位置索引，值是八叉树节点指针
 * @param pl_feat 原始特征点云
...
void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE*> &feat_map, pcl::PointCloud<PointType>::Ptr pl_feat, Eigen::Matrix3d R_p, Eigen::Vector3d t_p, int feattype, int fnum, int capacity)
{
  ...
  for(uint i=0; i<plsize; i++)
  {
    ...
    {
      //点在当前帧坐标系下的坐标放入体素中 fnum是当前关键帧的索引
      iter->second->plvec_orig[fnum]->push_back(pvec_orig);
      //点在世界坐标系下的坐标放入体素中 fnum是当前关键帧的索引
    ...
    else
    {
      ...
      //第一次新建八叉树节点 记录该体素中心点在世界坐标系下的位置 0.5是为了取到体素的中心点
...
int main(int argc, char **argv)
{
  ...
  int window_size = 20;
  int margi_size = 5;
  //? 体素滑窗内每个关键帧预留的点数 留空间用
  int filter_num = 1;
  //LM滑窗优化器 多线程计算大H矩阵 大J矩阵的线程数 就是分几份同时计算
  int thread_num = 4;
  //每隔几帧处理一次关键帧之间的关系
  int skip_num = 0;
  //?发布
  ...
  //?这里好像写错了 本意应该是判断特征拟合是否成功的阈值 第二次的是更严格的阈值
  ...
  while(n.ok())
  {
    ...
    //skip_num 每几帧向下处理一次，不向下处理的时候，q_gather_pose和t_gather_pose没有被初始化
    //第一帧一定是关键帧
    ...
    //把第一帧作为世界坐标系 q_poses和t_poses存储的是每个关键帧在世界坐标系下的位姿
    //第一个关键帧的处理
    if(plcount == 0)
    {
    ...
    if(plcount >= window_base+window_size)
    {
      ...
      //window_base是滑窗起始帧的索引
      //第一次不做优化，仅进行后面的边缘化，应该是防止整体位姿发生漂移
      if(window_base != 0)
      {
        //遍历自适应体素地图中的每个体素
        for(auto iter=surf_map.begin(); iter!=surf_map.end(); ++iter)
